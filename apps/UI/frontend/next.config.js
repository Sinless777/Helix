...
const nextConfig = {
  ...
  webpack(config, { isServer }) {
    ...
    const assetRule = config.module.rules.find((/** @type {{ test: { toString: () => string | string[]; }; }} */ rule) =>
      rule.test && rule.test.toString().includes('png|jpg|jpeg|gif|webp|avif|ico|bmp|svg')
    );
    if (assetRule) {
