...
describe('ToolsCards constant', () => {
  it('should be an array of CardProps objects', () => {
    expect(Array.isArray(ToolsCards)).toBe(true)
    expect(ToolsCards.length).toBeGreaterThan(0)
  })

  it('each card should match the CardProps interface', () => {
    ToolsCards.forEach((card: CardProps) => {
  ...
  it('matches the snapshot of ToolsCards', () => {
    expect(ToolsCards).toMatchSnapshot()
  })
})
