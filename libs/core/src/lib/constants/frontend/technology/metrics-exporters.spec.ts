...
describe('MetricsExportersCards constant', () => {
  it('should be an array of CardProps objects', () => {
    expect(Array.isArray(MetricsExportersCards)).toBe(true)
    expect(MetricsExportersCards.length).toBeGreaterThan(0)
  })

  it('each card should match the CardProps interface', () => {
    MetricsExportersCards.forEach((card: CardProps) => {
  ...
  it('matches the snapshot of MetricsExportersCards', () => {
    expect(MetricsExportersCards).toMatchSnapshot()
  })
})
