...
describe('FrameworksCards constant', () => {
  it('should be an array of CardProps objects', () => {
    expect(Array.isArray(FrameworksCards)).toBe(true)
    expect(FrameworksCards.length).toBeGreaterThan(0)
  })

  it('each card should match the CardProps interface', () => {
    FrameworksCards.forEach((card: CardProps) => {
  ...
  it('matches the snapshot of FrameworksCards', () => {
    expect(FrameworksCards).toMatchSnapshot()
  })
})
