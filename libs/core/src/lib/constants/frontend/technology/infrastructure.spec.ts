...
describe('InfrastructureCards constant', () => {
  it('should be an array of CardProps objects', () => {
    expect(Array.isArray(InfrastructureCards)).toBe(true)
    expect(InfrastructureCards.length).toBeGreaterThan(0)
  })

  it('each card should match the CardProps interface', () => {
    InfrastructureCards.forEach((card: CardProps) => {
  ...
  it('matches the snapshot of InfrastructureCards', () => {
    expect(InfrastructureCards).toMatchSnapshot()
  })
})
