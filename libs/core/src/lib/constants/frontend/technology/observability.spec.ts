...
describe('ObservabilityCards constant', () => {
  it('should be an array of CardProps objects', () => {
    expect(Array.isArray(ObservabilityCards)).toBe(true)
    expect(ObservabilityCards.length).toBeGreaterThan(0)
  })

  it('each card should match the CardProps interface', () => {
    ObservabilityCards.forEach((card: CardProps) => {
  ...
  it('matches the snapshot of ObservabilityCards', () => {
    expect(ObservabilityCards).toMatchSnapshot()
  })
})
